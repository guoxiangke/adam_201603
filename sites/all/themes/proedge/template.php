...
function proedge_form_element($variables){
  ...
  if (!empty($element['#type'])) {
    ...
    if ($element['#type'] == 'bef-link') {
      //       preg_match('/^(-)+/','-f-d-s',$matchs);
      // dpm(strlen($matchs[0]));
      preg_match('/>(-)+/', $element['#children'],$matchs);
      // dpm($element['#children']); return 'test';
      if(isset($matchs[0])){
        $attributes['class'][] = 'level-'.(strlen($matchs[0])-1);
      }else{
